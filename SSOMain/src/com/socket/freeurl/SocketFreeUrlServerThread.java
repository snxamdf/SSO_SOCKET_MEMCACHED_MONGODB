...
/**
 * SocketFreeUrlServerThread 免登URL服务线程
 */
public class SocketFreeUrlServerThread implements Runnable {
	private static final Logger LOGGER = Logger.getLogger(SocketFreeUrlServerThread.class.getName());
	private Socket socket = null;

	public SocketFreeUrlServerThread(Socket socket) {
		this.socket = socket;
	...
	private String generate(JSONObject json) {
		...
		if (bool) {
			...
			paramter += "&token=" + toKen + "&md5=" + md5 + (sessionId == null ? "" : "&sessionId=" + sessionId);
